...
async function createInfo() {
    ...
    await readLine().then(res => { connInfo.host = res; })
    console.log('Please input user')
    await readLine().then(res => { connInfo.user = res; })
    console.log('Please input password')
    await readLine().then(res => { connInfo.password = res; close() })
        ...
        dbconn.query(sql, (err, res) => {
            ...
            dbconn.query("CREATE DATABASE projectsMysql", (errs) => {
                ...
                projectsMysql.connect(err => {
                    if (err) throw err;
                    projectsMysql.query(`CREATE TABLE sysConfig (
                        config varchar(255) NOT NULL,
                        data varchar(255) DEFAULT NULL,
                        PRIMARY KEY (config)
                      ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;`, (errr, ress) => {
                        if (errr) throw errr;
                        console.log(ress)
                        ...
                        values.forEach(item => { data += `(${item}),` })
                        data = data.slice(0, -1)
                        projectsMysql.query(`INSERT INTO sysConfig(${columns}) VALUES ${data}`, (err, res) => {
                            if (err) throw err
                            console.log(res);
                            process.exit()
                        })
