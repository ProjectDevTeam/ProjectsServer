...
const selectValueName = (platform, item) => {
	...
	return new Promise((resolve, reject) => {
		conn.query(`select name from \`${platform}-value-list\` where id=${value}`, (err, data) => {
			if (err) error(err);
			resolve(`
            --${component ? component + '-' : ''}${group ? group + '-' : ''}${position ? position + '-' : ''}${status ? status + '-' : ''}${attribute ?? ''}:var(--${data[0].name});`);
		});
	});
};
